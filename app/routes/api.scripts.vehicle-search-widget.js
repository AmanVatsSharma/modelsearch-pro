  ...
  try {
    ...
    const widgetScript = `
      ...
      (function() {
        ...
        ready(function() {
          console.log('[Vehicle Search Widget] DOM ready, finding containers...');
          const containers = document.querySelectorAll('.vehicle-search-widget-container');
          console.log('[Vehicle Search Widget] Found ' + containers.length + ' containers');
        ...
        function loadYears(container, settings, modelId) {
          ...
          // Create the full API URL with proper app proxy format if needed
          const url = createApiUrl(apiBaseUrl, \`/api/vehicle/years?modelId=${modelId}\`, {
            t: timestamp
          });
        ...
        function loadSubmodels(container, settings, yearId) {
          ...
          // Create the full API URL with proper app proxy format if needed
          const url = createApiUrl(apiBaseUrl, \`/api/vehicle/submodels?yearId=${yearId}\`, {
            t: timestamp
          });
