...
/**
 * API route to get years for a specific model
...
export async function loader({ request }) {
  ...
  try {
    ...
    // Fetch years for this shop and model
    const years = await getYears(shop, modelId);
    console.log(`Found ${years.length} years for modelId ${modelId} in shop:`, shop);
    
    // Return response with CORS headers for admin extensions
    return cors(json({ years }));
  } catch (adminError) {
    ...
    try {
      ...
      try {
        // Pass the shop parameter to getYears
        const years = await getYears(shop, modelId);
        console.log(`Found ${years.length} years for modelId ${modelId} in unauthenticated shop:`, shop);
        ...
        console.error(`Error fetching years for modelId ${modelId} in unauthenticated shop:`, error);
        return json({ 
